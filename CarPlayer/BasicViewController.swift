...
class BasicViewController: UIViewController {
    ...
    @objc var _scrollView: UIScrollView = UIScrollView()
    ...
    @objc func createLetterLine() {
        ...
        sortedLetterKeys = Array(_letterToPos.keys).sorted(by: <)
        ...
        for letter in sortedLetterKeys {
            ...
            button.addTarget(self, action: #selector(BasicViewController.letterPressed(sender:)), for: .touchDown)
    ...
    //
    @objc func letterPressed(sender: UIButton!) {
        ...
        UIView.jumpSpringyToPosition(scrollView: _scrollView, pointToJumpTo: CGPoint(x: newPosInScrollView, y: 0.0))
    }
}
