...
class LyricsViewController: ModalViewController, UITextViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        _controller.setNotificationHandler(viewController: self, notificationFunctionName: #selector(LyricsViewController.setLyricsText))
        ...
        _lyricsTextView.isScrollEnabled = true

        _lyricsTextView.backgroundColor = UIColor.clear
        _lyricsTextView.textColor = UIColor.white
    ...
    func setLyricsText() {
        ...
        _closeButtonOnScrollView.frame = CGRect(x:CGFloat(xPosOfView), y:CGFloat(yPosOfView), width:CGFloat(widthOfView), height:heightOfCloseButton)
        _lyricsTextView.frame = CGRect(x:CGFloat(xPosOfView) + 10.0, y:CGFloat(yPosOfView) + 5.0, width:CGFloat(widthOfView) - 20.0, height:CGFloat(heightOfView) - 20.0)
        //DEBUG print("-------------")
        ...
        UIView.jumpSpringyToPosition(scrollView: _lyricsTextView, pointToJumpTo: CGPoint(x: 0.0, y: 0.0))

        if _realContentHeight >= CGFloat(self.heightOfView) {

            // Set the progress timer in order to be able to update the lyrics position:
            _progressTimer = Timer.scheduledTimer(
                timeInterval: _lyricsScrollFrequency,
                target: self,
    ...
    func updateLyricsPosition() {
        ...
        calculatePositionOnScrollview(currentProgressValue: currentProgressValue, position: &positionOnScrollView)

        // Slide smoothly to the new position:
        UIView.slideSmoothlyToPosition(scrollView: _lyricsTextView, pointToSlideTo: CGPoint(x: 0.0, y: positionOnScrollView))
    ...
    //
    func calculatePositionOnScrollview(currentProgressValue: Double, position: inout CGFloat) {
