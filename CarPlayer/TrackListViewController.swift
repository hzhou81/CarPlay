...
class TracklistViewController: ModalViewController {
    ...
    override func viewDidLoad() {
        ...
        _controller.setNotificationHandler(viewController: self, notificationFunctionName: #selector(TracklistViewController.createTrackList))
    ...
    func createTrackList() {
        ...
        if _initialSetup {
            ...
            closeButtonOnScrollView.addTarget(self,
                                              ...
                                              for: .touchUpInside)
            closeButtonOnScrollView.frame = CGRect(x:CGFloat(xPosOfView) + 10.0, y:CGFloat(yPosOfView) + 5.0, width:CGFloat(widthOfView) - 20.0, height:ySizeOfButton)
            scrollView.addSubview(closeButtonOnScrollView)

            // Set basic stuff for scrollview:
            scrollView.isScrollEnabled = true
            scrollView.frame = CGRect(x:CGFloat(xPosOfView), y:CGFloat(yPosOfView) + 5.0,
                                      width:CGFloat(widthOfView) - 20.0, height:CGFloat(heightOfView) - 20.0)
        }
        ...
        for track in _controller.currentPlaylist() {
            ...
            if (track.artwork != nil) {
                ...
                albumCoverImage.image = track.artwork!.image(at: sizeOfArtistImage)
                albumCoverImage.frame = CGRect(x:70, y:CGFloat(curYPos) + 10,
                                               width:CGFloat(MyBasics.TrackListView_ArtistImage_Width), height:CGFloat(MyBasics.TrackListView_ArtistImage_Height))
            ...
            button.setTitle(track.title!, for: .normal)
            button.titleLabel!.font = MyBasics.fontForMediumText
            button.setTitleColor(labelTextColor, for: UIControlState.normal)
            ...
            button.sizeToFit()
            button.addTarget(self, action: #selector(TracklistViewController.trackNameTapped(sender:)), for: UIControlEvents.touchUpInside)
            button.setMediaItem(mediaItem: track)
        ...
        if _initialSetup {
            ...
            scrollView.contentSize = CGSize(width:
                CGFloat(widthOfView) - 20.0, height:CGFloat(curYPos + yPosOfView));
    ...
    //
    func tweakYPosOfNowPlayingItem(totalHeightOfScrollView: Int, scrollView: UIScrollView, yPosOfNowPlayingItem: inout CGFloat) {
    ...
    func jumpToPosition(yPosOfNowPlayingItem: CGFloat) {
        ...
        UIView.jumpSpringyToPosition(scrollView: scrollView, pointToJumpTo: CGPoint(x: 0.0, y: yPosOfNowPlayingItem))
    }
    ...
    func trackNameTapped(sender: UIButtonWithFeatures) {
      ...
      //  var trackName: String = sender.mediaItem().title!
        ...
        _controller.setTrack(track: sender.mediaItem())
